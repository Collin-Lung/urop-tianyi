...
for monomer in final_p5:
    if monomer == '1':
        fss_p5.append(monomer.replace('1', '21.697'))
    elif monomer == '2':
        fss_p5.append(monomer.replace('2', '29.613'))
    elif monomer == '3':
        fss_p5.append(monomer.replace('3', '73.735'))
    elif monomer == '4':
        fss_p5.append(monomer.replace('4', '30.415'))

# Combine HLB data and FSS data
hlb_fss_p5 = HLB_p5 + fss_p5

x = np.reshape(hlb_fss_p5, (200, 9216)).T
df = pd.DataFrame(x)
...
plt.scatter(p5_pca[:, 0], p5_pca[:, 1], alpha=0.20, s=10)
plt.title('HLB & FSS p5 PCA', fontsize=25)
